...
begin
  ...
  try
    ...
    if g_pConfig.m_nPunishIntervalRate <= 0.10 then begin
...
begin
  if not m_ShowOpen then
    Exit;
  with g_pConfig, (Sender as TSpinEdit) do begin
    case Tag of
...
begin
  with g_pConfig, (Sender as TCheckBox) do begin
    case Tag of
...
var
  nMagID                    : Integer;
begin
  with g_pConfig, (Sender as TComboBox) do begin
    case Tag of
      200: begin
...
begin
  ...
  if FileCtrl.SelectDirectory('选择文件机器吗黑名单文件（BlockHWID.txt）', szFolder, szDir, [sdNewUI, sdShowFiles], TWinControl(Self)) then begin
...
begin
  ...
  for i := 0 to MemoCmdFilter.Lines.Count - 1 do begin
  ...
  g_AbuseList.Clear;
  for i := 0 to ListBoxAbuseFilterText.Items.Count - 1 do begin
  ...
  if m_nDefItemSpeed <> g_pConfig.m_nMaxItemSpeed then begin
...
begin
  if not m_ShowOpen then
    Exit;
  with g_pConfig, (Sender as TEdit) do begin
    case Tag of
...
var
  ...
  szIPaddr                  : string;
begin
  if (ListBoxBlockList.Items.Count > 0) then begin
    for i := 0 to ListBoxBlockList.Items.Count - 1 do begin
      szIPaddr := ListBoxBlockList.Items.Strings[i];
      if ListBoxTempList.Items.IndexOf(szIPaddr) < 0 then begin
...
begin
  ...
  for n := 0 to USER_ARRAY_COUNT - 1 do begin
    ...
    if (UserObj <> nil) and (UserObj.m_tLastGameSvr <> nil) and
      (UserObj.m_tLastGameSvr.Active) and not UserObj.m_fKickFlag then begin
...
var
  ...
  UserObj                   : TSessionObj;
begin
  if (ListBoxActiveList.Items.Count > 0) then begin
    ...
    for i := 0 to ListBoxActiveList.Items.Count - 1 do begin
      ...
      if (UserObj <> nil) and (UserObj.m_tLastGameSvr <> nil) and (UserObj.m_tLastGameSvr.Active) then begin
        fExists := False;
        for ii := 0 to ListBoxBlockList.Items.Count - 1 do begin
          if Integer(ListBoxBlockList.Items.Objects[ii]) = UserObj.m_pUserOBJ.nIPaddr then begin
            fExists := True;
            Break;
          end;
        end;
...
begin
  if (ListBoxAbuseFilterText.ItemIndex >= 0) and (ListBoxAbuseFilterText.ItemIndex < ListBoxAbuseFilterText.Items.Count) then begin
...
begin
  ...
  for i := 0 to g_PunishList.Count - 1 do begin
...
var
  szHWID                    : string;
begin
  with ListBoxBlockHWIDList do begin
    szHWID := '';
...
var
  ...
  xHWID                     : MD5.MD5Digest;
begin
  if (ListViewCurHWIDList.ItemIndex >= 0) and (ListViewCurHWIDList.ItemIndex < ListViewCurHWIDList.Items.Count) then begin
...
var
  ...
  UserObj                   : TSessionObj;
begin
  if (ListViewCurHWIDList.Items.Count > 0) then begin
    if Application.MessageBox(
      ...
      MB_OKCANCEL + MB_ICONQUESTION) <> IDOK then
      Exit;
    for i := 0 to ListViewCurHWIDList.Items.Count - 1 do begin
...
begin
  ...
  if (dwIPLow = INADDR_NONE) then begin
    MessageBox(0, '输入的低位IP格式错误', '错误', MB_OK + MB_ICONERROR);
    Exit;
  end;
  if (dwIPHigh = INADDR_NONE) then begin
    MessageBox(0, '输入的高位IP格式错误', '错误', MB_OK + MB_ICONERROR);
    Exit;
  end;

      if dwIPLow > dwIPHigh then begin
        dwtmp := dwIPLow;
        dwIPLow := dwIPHigh;
        dwIPHigh := dwtmp;
      end;
  ...
  for i := 0 to g_BlockIPAreaList.Count - 1 do begin
    if PInt64(pIPArea)^ = PInt64(ListBoxIPAreaFilter.Items.Objects[i])^ then begin
...
var
  i                         : Integer;
begin
  with ListBoxIPAreaFilter do begin
    if (ItemIndex >= 0) and (ItemIndex < Items.Count) then begin
      if (ItemIndex < g_BlockIPAreaList.Count) and
        (PInt64(g_BlockIPAreaList.Objects[ItemIndex])^ = PInt64(Items.Objects[ItemIndex])^) then begin
        g_BlockIPAreaList.Delete(ItemIndex);
        Dispose(PInt64(g_BlockIPAreaList.Objects[ItemIndex]));
      end else begin
        for i := 0 to g_BlockIPAreaList.Count - 1 do begin
          if PInt64(g_BlockIPAreaList.Objects[i])^ = PInt64(Items.Objects[ItemIndex])^ then begin
...
begin
  ...
  for n := 0 to USER_ARRAY_COUNT - 1 do begin
    UserObj := g_UserList[n];
    if (UserObj <> nil) and (UserObj.m_tLastGameSvr <> nil) and (UserObj.m_tLastGameSvr.Active) then begin
      if CompareText(szChrName, UserObj.m_szTrimChrName) = 0 then begin
        ListBoxSpeedLimitList.Items.AddObject(szChrName, TObject(UserObj));
        g_PunishList.AddObject(szChrName, TObject(UserObj));
        UserObj.m_fSpeedLimit := True;
        Exit;
...
var
  ...
  UserObj                   : TSessionObj;
begin
  if (ListBoxSpeedLimitList.ItemIndex >= 0) and (ListBoxSpeedLimitList.ItemIndex < ListBoxSpeedLimitList.Items.Count) then begin
...
var
  ...
  szChrName, szIPaddr       : string;
begin
  if (0 < ListBoxActiveList.Items.Count) then begin
    if Application.MessageBox(
...
var
  ...
  szIPaddr                  : string;
begin
  if (ListBoxTempList.Items.Count > 0) then begin
    for i := 0 to ListBoxTempList.Items.Count - 1 do begin
      szIPaddr := ListBoxTempList.Items.Strings[i];
      if ListBoxBlockList.Items.IndexOf(szIPaddr) < 0 then begin
...
begin
  if (0 < ListBoxActiveList.Items.Count) then begin
    ...
    for i := 0 to ListBoxActiveList.Items.Count - 1 do begin
      szIPaddr := ListBoxActiveList.Items.Strings[i];
      szIPaddr := Trim(GetValidStr3(szIPaddr, szChrName, [' ']));
      if ((szIPaddr = '') or (szIPaddr = Char(15))) and (szChrName <> '') then begin
        nIPaddr := inet_addr(PChar(szChrName));
        if nIPaddr <> INADDR_NONE then begin
          if ListBoxTempList.Items.IndexOf(szChrName) < 0 then begin
...
begin
  ...
  if ListBoxAbuseFilterText.Items.IndexOf(sInputText) >= 0 then begin
...
begin
  ...
  if (nSelectIndex >= 0) and (nSelectIndex < ListBoxAbuseFilterText.Items.Count) then begin
  ...
  if ListBoxAbuseFilterText.ItemIndex < 0 then begin
...
begin
  if (ListBoxActiveList.ItemIndex >= 0) and (ListBoxActiveList.ItemIndex < ListBoxActiveList.Items.Count) then begin
    ...
    if nIPaddr <> INADDR_NONE then begin
      fExists := False;
      for i := 0 to ListBoxTempList.Items.Count - 1 do begin
        if Integer(ListBoxTempList.Items.Objects[i]) = nIPaddr then begin
          fExists := True;
          Break;
        end;
      end;
...
begin
  if (ListBoxActiveList.ItemIndex >= 0) and (ListBoxActiveList.ItemIndex < ListBoxActiveList.Items.Count) then begin
    ...
    if nIPaddr <> INADDR_NONE then begin
      fExists := False;
      for i := 0 to ListBoxBlockList.Items.Count - 1 do begin
        if Integer(ListBoxBlockList.Items.Objects[i]) = nIPaddr then begin
          fExists := True;
          Break;
        end;
      end;
...
begin
  if not m_ShowOpen then
    Exit;
  with g_pConfig, (Sender as TTrackBar) do begin
    if Tag = 0 then begin
      ...
      if cbSyncSpeedRate.Checked then begin
      ...
      if cbSyncSpeedRate.Checked then begin
        ...
        if (cbxMagicList.Items.Count > 0) and (cbxMagicList.ItemIndex >= 0) then begin
...
begin
  if (ListBoxTempList.ItemIndex >= 0) and (ListBoxTempList.ItemIndex < ListBoxTempList.Items.Count) then begin
    ...
    if (ListBoxTempList.ItemIndex < g_TempBlockIPList.Count) and (g_TempBlockIPList[ListBoxTempList.ItemIndex] = szIPaddr) then begin
      g_TempBlockIPList.Delete(ListBoxTempList.ItemIndex);
    end else begin
      for i := 0 to g_TempBlockIPList.Count - 1 do begin
        if g_TempBlockIPList.Strings[i] = szIPaddr then begin
...
begin
  if (ListBoxBlockList.ItemIndex >= 0) and (ListBoxBlockList.ItemIndex < ListBoxBlockList.Items.Count) then begin
    ...
    if (ListBoxBlockList.ItemIndex < g_BlockIPList.Count) and (g_BlockIPList[ListBoxBlockList.ItemIndex] = szIPaddr) then begin
      g_BlockIPList.Delete(ListBoxBlockList.ItemIndex);
    end else begin
      for i := 0 to g_BlockIPList.Count - 1 do begin
        if g_BlockIPList.Strings[i] = szIPaddr then begin
