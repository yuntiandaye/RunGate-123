...
uses
  Windows, dialogs, Messages, SysUtils, Classes, ClientThread,
  AcceptExWorkedThread, IOCPTypeDef, ThreadPool,
  Protocol, MD5, SHSocket, SyncObj;
...
uses
  ...
  ChatCmdFilter, AbusiveFilter, Punishment, Filter;
    ...
    while m_xMsgList.Count > i do begin
      ...
      if nCmd = CM_HIT then begin
        if (iCmd = CM_HIT) or
          ...
          (iCmd = CM_SMITELONGHIT3) then begin
          Result := True;
          Break;
        end else
          Inc(i);
    ...
    if DelayMsg.nBufLen > 0 then begin
      ...
      case DelayMsg.nCmd of
        ...
        CM_HIT, CM_HEAVYHIT, CM_BIGHIT, CM_POWERHIT,
          ...
          CM_PURSUEHIT, CM_SMITELONGHIT2, CM_SMITELONGHIT3: begin
...
begin
  ...
  if (Len >= 5) and g_pConfig.m_fDefenceCCPacket then begin
    if StrPos(PChar(Addr), 'HTTP/') <> nil then begin
...
//    g_pJxLogger.WriteLog('client msgid:%d,%d', [CltCmd.Cmd, Integer(m_Stat)]);
    ...
    case CltCmd.Cmd of
      ...
      CM_SPELL: if g_pConfig.m_fSpellInterval then begin //1380
          dwCurrentTick := GetTickCount();
          if CltCmd^.Magic >= MAGIC_NUM then begin
            fPacketOverSpeed := False;
            Exit;
          end else if MAIGIC_DELAY_ARRAY[CltCmd^.Magic] then begin //过滤武士魔法
            fPacketOverSpeed := False;
            if m_fSpeedLimit then
            ...
            if (nInterval >= nSpellInterval) then begin
              if MAIGIC_ATTACK_ARRAY[CltCmd^.Magic] then begin
      ...
      CM_BUTCH: begin
          if g_pConfig.m_fButchInterval then begin
            ...
            if nInterval >= g_pConfig.m_nButchInterval then begin
              ...
              fPacketOverSpeed := True;
              if g_pConfig.m_tOverSpeedPunishMethod = ptDelaySend then begin
                if not PeekDelayMsg(CltCmd.Cmd) then begin
      ...
      CM_TURN: if g_pConfig.m_fTurnInterval and (g_pConfig.m_tOverSpeedPunishMethod <> ptTurnPack) then begin
          if m_dwLastDirection <> CltCmd.Dir then begin
            fPacketOverSpeed := False;
            dwCurrentTick := GetTickCount();
      ...
      CM_OPENSTALL: begin
          dwCurrentTick := GetTickCount();
          if (CltCmd.Series = 0) then begin //取消摆摊
            m_dwOpenStoreTick := dwCurrentTick;
          end else begin
            if (dwCurrentTick - m_dwOpenStoreTick >= 12000) then begin
              m_dwOpenStoreTick := dwCurrentTick;
            end else begin
              if (dwCurrentTick - m_dwWaringTick > 1000) then begin
                m_dwWaringTick := dwCurrentTick;
      ...
      CM_UPDATESTALLITEM: begin //相当于修正了摆摊改价格问题
          dwCurrentTick := GetTickCount();
          if (CltCmd.Series > 0) then begin
            if (dwCurrentTick - m_dwWaringTick > 1000) then begin
              m_dwWaringTick := dwCurrentTick;
      ...
      CM_SAY: begin
          if g_pConfig.m_fChatInterval then begin
            if PChar(nABuf + SizeOf(TCmdPack))^ <> '@' then begin
              dwCurrentTick := GetTickCount();
          ...
          if nDeCodeLen > SizeOf(TCmdPack) then begin
            if PChar(nABuf + SizeOf(TCmdPack))^ = '@' then begin
              ...
              end else if PChar(nABuf + SizeOf(TCmdPack))^ <> '@' then begin
      ...
      CM_HEROEAT: if g_pConfig.m_fEatInterval and (CltCmd.Direct in [0, 1, 3]) then begin
          dwCurrentTick := GetTickCount();
      ...
      CM_EAT: if g_pConfig.m_fEatInterval then begin
          if CltCmd.Direct in [0, 1, 3] then begin
            dwCurrentTick := GetTickCount();
...
begin
  if (Len < FIRST_PAKCET_MAX_LEN) and (Len > 15) then begin
    if (PChar(Addr)[0] <> '*') or (PChar(Addr)[1] <> '*') then begin
    ...
    if (Length(szAccount) > 4) and (Length(szAccount) <= 12) and (Length(szCharName) > 2) and (Length(szCharName) < 15) then begin
      ...
      if (Length(szClientVerNO) <> 9) then begin
        Succeed := False;
        Exit;
      end;
