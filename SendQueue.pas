...
const
  SEND_BUFFER_LEN   = 48 * 1024;
  SEND_BLOCK_LEN    = 08 * 1024;
...
begin
  ...
  with PTRSendObj^ do begin
    ...
    //try
      //数据包太大，丢掉
    if (nSize > 0) and (nSize <= SEND_BUFFER_LEN) then begin
        //如果动态列表有数据，继续加到动态列表，取出列表前端数据加到缓冲，保证顺序
      DynPacket := GetDynPacket(ComOBJ);
      if DynPacket <> nil then begin
        New(NewDynBuf);
        NewDynBuf.Pos := 0;
        NewDynBuf.Len := nSize;
      ...
      if BufLPos = 0 then begin
        if RightLen >= Size then begin
          Move(InBuffer^, DPBuffer^, Size);
          Dec(RightLen, Size);
          Inc(BufRPos, Size);
          Inc(DPBuffer, Size);
          AddBufferOK := True;
        end else if LeftLen >= Size then begin
        ...
        //取的动态数据加到缓冲成功，释放内存
      if IsDynPacket and AddBufferOK then begin
  ...
  if QuerySend then begin
    if not PostIOCPSend(ComOBJ) then begin
...
begin
  ...
  with PTRSendObj^ do begin
    ...
    if (nSize > 0) and (nSize <= SEND_BUFFER_LEN) then begin
      if BufLPos = 0 then begin
        if RightLen >= nSize then begin
          Move(pInBuffer^, DPBuffer^, nSize);
          Dec(RightLen, nSize);
          Inc(BufRPos, nSize);
          Inc(DPBuffer, nSize);
        end else if LeftLen >= nSize then begin
          Move(pInBuffer^, BPBuffer^, nSize);
          DPBuffer := PChar(Integer(BPBuffer) + nSize);
          BufLPos := nSize;
        end;
      end else begin
        if (LeftLen - BufLPos) >= nSize then begin
          Move(pInBuffer^, DPBuffer^, nSize);
          Inc(BufLPos, nSize);
          Inc(DPBuffer, nSize);
        end;
    ...
    if InSend then begin
      ...
      if iSendLen > 0 then begin
        ...
        if BufLPos > 0 then begin
          ...
          DynPacket := GetDynPacket(ComOBJ);
          if DynPacket <> nil then begin
            Result := AddBuffer2(ComOBJ, DynPacket.Buf, DynPacket.Len);
            FreeMem(DynPacket.Buf);
            Dispose(DynPacket);
            DynSendList.Delete(0);
            LeaveCriticalSection(QueueLock);
