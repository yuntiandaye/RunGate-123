...
uses
  ...
  IOCPTypeDef, SendQueue, ClientThread, FixedMemoryPool, IniFiles;

const
  //MaxUser
  MAX_GAME_USER     : Integer = 2500;
  MAX_LOGIN_USER    : Integer = 2500 + 48;
  USER_ARRAY_COUNT  : Integer = 2500 + 48;
  ...
  RECV_BUFFER_LEN   = 32 * 1024;
  RECV_PACKET_LEN   = 04 * 1024;
...
type
  ...
  _tagUserObj = record
    ...
    nIPLen: Integer;
    nIPAddr: Integer;
    pszIPAddr: array[0..24 - 1] of Char;
    Port: Integer;
  end;
  ...
  public
    ...
    function AddUser(const SendObj: PIOCPCommObj; const CIP: PChar; const NIP: Integer; const CPort: Integer): PUserOBJ;
...
uses
  ClientSession, LogManager, Filter, ConfigManager, Protocol;
...
begin
  if IOCPData.WBuf.Len = IOLength then //发送完成，再取缓冲区数据继续投递
    Result := FSendSocket.HandleSendFinished(IOCPData)
  else begin //未发送完，继续投递剩余数据
    ...
    while True do begin
      ...
      if (PIOData <> nil) then begin
        ...
        if fQueued then begin
          case IOCPData.WorkType of
            SOCKET_READ: begin
                if dwBytesTransferred > 0 then begin
...
var
  ...
  UserOBJ           : PUserOBJ;
  ServerOBJ         : PServerInfo;
  szRemoteIP        : array[0..127] of AnsiChar;
begin
  bClose := False;
  ServerOBJ := PServerInfo(IOKey);
  try
    //与服务器连接未激活
    if not ServerOBJ.pClient.Active then begin
      bClose := True;
      Exit;
    end;
    ...
    FGetAcceptExSockaddrsFunc(
      ...
      //  g_pLogMgr.Add(Format('RemoteSockaddrLen: %d', [RemoteSockaddrLen]));
...
//    szRemoteIP[StrLen(FClientIP)] := #0;
    ...
    if Filter.IsBlockIP(pRemoteSockaddr.sin_addr.S_addr) then begin

      FClientIP := inet_ntoa(pRemoteSockaddr.sin_addr);
      StrFmt(@szRemoteIP[0], '%s', [FClientIP]);
    ...
    if Filter.IsBlockIPArea(pRemoteSockaddr.sin_addr.S_addr) then begin
    ...
    if Filter.OverConnectOfIP(pRemoteSockaddr.sin_addr.S_addr) then begin

      FClientIP := inet_ntoa(pRemoteSockaddr.sin_addr);
      StrFmt(@szRemoteIP[0], '%s', [FClientIP]);

      if g_pLogMgr.CheckLevel(5) then
        g_pLogMgr.Add(Format('超过每IP连接[%d]: %s', [g_pConfig.m_nMaxConnectOfIP, szRemoteIP]));
      bClose := True;
      Exit;
    end;
    ...
    if FSendSocket.AddLoginUser(IOCPData, WriterOBJ) then begin
      ...
      UserOBJ := FUserManager.AddUser(WriterOBJ, @szRemoteIP[0], pRemoteSockaddr.sin_addr.S_addr, FClientPort);
      if UserOBJ <> nil then begin
        ...
        if not FRecvSocket.AddLoginUser(PTRLoginObj, ReaderOBJ) then begin
          ...
          if g_pLogMgr.CheckLevel(7) then
            g_pLogMgr.Add(Format('登陆游戏: %s', [szRemoteIP]));
        end;
      end else
        bClose := True;
    end else
    ...
    PostAcceptEx(ServerOBJ.nSocket, PLoginObj(IOCPData)); //继续投递下一个IO请求
  end;
end;
...
begin
  ...
  if NewSocket = INVALID_SOCKET then begin
    //g_pLogMgr.Add('11111111111111111111111111111111111111');
  ...
  if LoginObj <> nil then
    ...
    if NewObjIndex = 0 then begin
      HandleException(Format('当前用户数太多:%d', [FAcceptExPool.InUseBlock]));
      Exit;
    end;
...
begin
  ...
  if MemID <> 0 then begin
    ...
    with ReaderOBJ^ do begin
      ...
      tCommObj.WBuf.Buf := @ReaderOBJ.pszBuffer[0];
    end;
...
begin
  ...
  with IOCPData^, PTRRecvOBJ^ do begin
    if (IOLen >= PTRRecvOBJ.dwRecvPos) then begin
      WBuf.Buf := @pszBuffer;
      ...
      else if dwRecvPos >= RECV_BUFFER_LEN then begin
        Result := False;
        Exit;
      end;
...
function TUserManager.AddUser(const SendObj: PIOCPCommObj; const CIP: PChar; const NIP: Integer; const CPort: Integer): PUserOBJ;
var
  dwUserID          : DWORD;
begin
...
begin
  ...
  if fFreeMem then begin
    try
      ...
      on E: Exception do begin
        if g_pLogMgr.CheckLevel(5) then begin
...
initialization
  tempIni := TIniFile.Create(_STR_CONFIG_FILE);
  if tempIni.ReadInteger('Integer', 'MaxUser', -1) <= 500 then
    tempIni.WriteInteger('Integer', 'MaxUser', 500);
  MAX_GAME_USER := tempIni.ReadInteger('Integer', 'MaxUser', 2500);
  tempIni.Free;
