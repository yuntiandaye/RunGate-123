...
uses
  ...
  WinSock2, ThreadPool, Protocol, ExtCtrls;
...
begin
  ...
  //try
  for i := 0 to m_xTempUserList.Count - 1 do begin
    if not g_fServiceStarted then
      Break;
    CSession := TSessionObj(m_xTempUserList[i]);
    if (CSession <> nil) and (CSession.m_tLastGameSvr <> nil) then begin
        //单向包验证

      if not CSession.m_fKickFlag then begin
        CSession.ProcessDelayMsg();
        if (CSession.m_fHandleLogin < 3) then begin
          if (GetTickCount - CSession.m_dwClientTimeOutTick > g_pConfig.m_nClientTimeOutTime) then begin
...
begin
  ...
  try
    for i := 0 to FormMain.m_xGameServerList.GameServerCount - 1 do begin
...
begin
  ...
  nRow := 1;
  for i := 0 to IOCPAccepter.ServerCount - 1 do begin
    if IOCPAccepter.Active then begin
...
begin
  ...
  for i := 0 to USER_ARRAY_COUNT - 1 do begin
    ...
    if (tUserObj <> nil) and (tUserObj.m_tLastGameSvr <> nil) and (tUserObj.m_tLastGameSvr.Active) and not tUserObj.m_fKickFlag then begin
...
begin
  case idEvent of
    _IDM_TIMER_STARTSERVICE: begin
        KillTimer(g_hMainWnd, _IDM_TIMER_STARTSERVICE);
        StartService();
      end;
    _IDM_TIMER_STOPSERVICE: begin
        KillTimer(g_hMainWnd, _IDM_TIMER_STOPSERVICE);
        StopService();
      end;
    _IDM_TIMER_KEEP_ALIVE: begin
        KeepAlive();
      end;
    _IDM_TIMER_THREAD_INFO: begin
        ShowThreadInfo();
      end;
    _IDM_TIMER_BROADCAST_USER_ITEM_SPEED: begin
        KillTimer(g_hMainWnd, _IDM_TIMER_BROADCAST_USER_ITEM_SPEED);
        BroadCastUserItemSpeed();
      end;
    _IDM_TIMER_BROADCAST_CLIENT_ACTION_SPEED: begin
