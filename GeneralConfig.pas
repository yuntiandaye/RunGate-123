...
type
  TfrmGeneralConfig = class(TForm)
    ...
    procedure cbAddLogClick(Sender: TObject);
  private
...
procedure TfrmGeneralConfig.cbAddLogClick(Sender: TObject);
begin
  if not m_Showed then
    Exit;
  with g_pConfig, (Sender as TCheckBox) do begin
    InterlockedExchange(Integer(m_fAddLog), Integer(Checked));
  end;
end;
...
begin
  ...
  try
    ...
    nIndex := speGateIdx.Value;
    if nIndex in [Low(g_pConfig.m_xGameGateList)..High(g_pConfig.m_xGameGateList)] then begin
