...
begin
  ...
  Lock();
  try
    for i := 0 to m_xDenyList.Count - 1 do begin
      pHWIDCnt := pTHWIDCnt(m_xDenyList[i]);
      if MD5.MD5Match(pHWIDCnt.HWID, HWID) then begin
        Result := i;
        Exit;
      end;
    end;
...
begin
  ...
  try
    ...
    for i := 0 to m_xCurList.Count - 1 do begin
      pHWIDCnt := pTHWIDCnt(m_xCurList[i]);
      if MD5.MD5Match(pHWIDCnt.HWID, HWID) then begin
        if pHWIDCnt.Count + 1 > g_pConfig.m_nMaxClientCount then begin
          Result := True;
    ...
    for i := 0 to m_xCurList.Count - 1 do begin
      ...
      if MD5.MD5Match(pHWIDCnt.HWID, HWID) then begin
        if pHWIDCnt.Count > 0 then
          Dec(pHWIDCnt.Count);
        if pHWIDCnt.Count = 0 then begin
          Dispose(pHWIDCnt);
          m_xCurList.Delete(i);
        end;
        Break;
...
//  EnterCriticalSection(g_csBlockedIPLock);
//  try
    if g_BlockIPList.IndexOf(szIP) < 0 then begin
...
//  try
    fExists := False;
    for i := 0 to g_BlockIPList.Count - 1 do begin
      if Integer(g_BlockIPList.Objects[i]) = nIP then begin
        fExists := True;
        Break;
      end;
    end;
...
//  EnterCriticalSection(g_csBlockedIPLock);
//  try
    if g_TempBlockIPList.IndexOf(szIP) < 0 then begin
      nIP := inet_addr(PChar(szIP));
      if nIP <> INADDR_NONE then
...
//  try
    ...
    for i := 0 to g_TempBlockIPList.Count - 1 do begin
      if Integer(g_TempBlockIPList.Objects[i]) = nIP then begin
        fExists := True;
        Break;
      end;
    end;
...
//  EnterCriticalSection(g_csBlockedIPLock);
//  try
    if g_BlockIPList.Count > 0 then begin
      for i := 0 to g_BlockIPList.Count - 1 do begin
        if nRemoteIP = Integer(g_BlockIPList.Objects[i]) then begin
          Result := True;
          Exit;
        end;
      end;
    end;
    if g_TempBlockIPList.Count > 0 then begin
      for i := 0 to g_TempBlockIPList.Count - 1 do begin
        if nRemoteIP = Integer(g_TempBlockIPList.Objects[i]) then begin
          Result := True;
          Break;
        end;
      end;
...
begin
  ...
  try
    for i := 0 to g_ConnectOfIPList.Count - 1 do begin
      PerIPAddr := pTPerIPAddr(g_ConnectOfIPList[i]);
      if PerIPAddr.IPaddr = Addr then begin
    ...
    for i := 0 to g_ConnectOfIPList.Count - 1 do begin
      ...
      if PerIPAddr.IPaddr = Addr then begin
        if PerIPAddr.Count > 0 then
          Dec(PerIPAddr.Count);
        if PerIPAddr.Count <= 0 then begin
          Dispose(PerIPAddr);
...
//  EnterCriticalSection(g_csBlockedIPLock);
//  try
    if g_BlockIPAreaList.Count > 0 then begin
      dwReverseIP := Misc.ReverseIP(DWORD(nRemoteIP));
      for i := 0 to g_BlockIPAreaList.Count - 1 do begin
        pIPArea := pTIPArea(g_BlockIPAreaList.Objects[i]);
        if (dwReverseIP >= pIPArea.Low) and (dwReverseIP <= pIPArea.High) then begin
          Result := True;
          Exit;
        end;
      end;
